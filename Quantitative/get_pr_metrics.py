...
def gh_get(url, params=None, max_retries=3):
    # Small pause before every request to avoid tripping GitHub's secondary
    # rate limit, which fires on rapid bursts even under the 5,000/hour cap.
    time.sleep(0.5)
...
def main():
    ...
    # Acceptance is defined as merged (non-null merged_at), not merely closed —
    # a rejected PR is also "closed" without being accepted, so using
    # closed_or_merged here silently inflates the rate. See Section 3.3.1.
    acceptance_rate = (merged_count / total) if total else float("nan")
